...
def main():
  ...
  #  model.eval()
    ...
    for j in range(len(data_schedule)):
        model = ResNet18(nclass=nclass, scale=args.model_scale, channels=nchannels, **kwargsUser).to(device)
        model_saved = torch.load(f"{saved_model_path}/{j+5}_Saved_Model_with_{data_schedule[j]}_CIFAR100_Data_0621_13_24_49", map_location=device)
        model.load_state_dict(model_saved)
                ...
                for k in range(nclass):
                    ...
                    if i!=k:
                       ...
                       # if i == 6:
                        ...
                        with open('{}/Iterative_CIFAR100_split6_7_Until_Low_Loss_BOUNDARY_PROBS_{}.txt'.format(model_dir, date_time),
                                  'a') as f:
                            f.write(
    ...
    for t in range(1):
        ...
        plt.legend()
        plt.savefig(f"{model_dir}/../PrototypeEvaluation_ResNet18_CIFAR10/metric_plots/{date_time}_CIFAR100_split6_7Batch{t}.png")
        plt.show()
        plt.figure().clear()
        plt.close()
        plt.cla()
