...
def main():
  ...
  #  model.eval()
      ...
      #  for proto in par_image_tensors:
        ...
        for t in range(1):
          ...
          #  stacked_trained_l2.append(torch.stack(batch_l2_trained_diff, dim=0))
            ...
            torch.save(torch.stack(set_latent_boundaries, dim = 0), f"{saved_boundaries_path}/Final3_{data_schedule[j]}7_Boundaries_Latents_{date_time}.pt")
            torch.save(torch.stack(set_trained_boundaries, dim=0), f"{saved_boundaries_path}/Final3_{data_schedule[j]}7_Boundaries_Images_{date_time}.pt")
    ...
    with open('{}/Iterative_CIFAR100_split3_Until_Low_Loss_BOUNDARY_{}.txt'.format(model_dir, date_time), 'a') as f:
