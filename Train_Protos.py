...
def main():
    ...
    for j in range(len(data_schedule)):
        ...
        model_saved = torch.load(f"{saved_model_path}/{j}_Saved_Model_with_{data_schedule[j]}_0621_16_53_47", map_location=device)
        ...
        for run in range(args.total_runs):
            for epoch in range(1,7):
                ...
                if epoch == 0:
                    print(last_loss)
        torch.save(par_image_tensors, f"{saved_protos_path}/Final_Saved_Protos_SPLIT_{j}")
