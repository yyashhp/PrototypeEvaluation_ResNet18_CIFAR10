  ...
  #  model.eval()
   ...
   #     print(f"Sizes of boundary latent loaded: {boundary_latents.shape}")
                ...
                for k in range(nclass):
                    ...
                    if i!=k:
                       ...
                       # if i == 6:
                        ...
                        with open('{}/CIFAR100_BATCH3split5_6Until_Low_Loss_BOUNDARY_PROBS_{}.txt'.format(model_dir, date_time),
                                  'a') as f:
                            f.write(
    ...
    for t in range(1):
        ...
        plt.legend()
        plt.savefig(f"{model_dir}/../PrototypeEvaluation_ResNet18_CIFAR10/metric_plots/{date_time}_CIFAR100_Batch3_Splits5_6Data{t}.png")
        plt.show()
        plt.figure().clear()
        plt.close()
        plt.cla()
