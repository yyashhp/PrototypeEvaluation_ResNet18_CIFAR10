...
parser.add_argument('--schedule', nargs='+', type=float, default=[0.1, 0.25, 0.4, 0.6, 0.7, 0.8, 1.0],
...
def main():
    ...
    for run in range(args.total_runs):
        with torch.no_grad():
            par_images_metric = torch.rand([kwargsUser['num_classes'], nchannels, H, W], dtype=torch.float,
                                           device=device)
    ...
    for j in range(len(splits)):
        ...
        with open('{}/train_hist_CIFAR100{}.txt'.format(model_dir, date_time), 'a') as f:
            f.write("=====INITIAL======\n")
            f.write("{0:4.3f}\t{1:4.3f}\t{2:4.0f}\t{3:4.3f}\t{4:6.5f}\n".format(initial_acc_train, initial_acc_test,
                                                                                len(train_loader.dataset),
                                                                                initial_loss_train, initial_loss_test))
            f.write("==================\n")
        f.close()
        ...
        if j == 0:
            ...
            optimizer = optim.SGD(model.parameters(), lr=0.05, momentum=args.momentum, weight_decay=args.weight_decay,
                                  nesterov=True)
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs * len(train_loader),
                                                         eta_min=0.0000001, last_epoch=-1, verbose=False)
        criterion = nn.CrossEntropyLoss()
        for epoch in range(1, args.epochs + 1):
            model.train()
            model.multi_out = 1
            train(model=model, device=device, epoch=epoch, max_steps=steps_per_epoch, scheduler=scheduler,
                  criterion=criterion, cur_loader=train_loader, optimizer=optimizer, transformDict=transformDict)
        ...
        torch.save(model.state_dict(), f"{saved_model_path}/{j}_Saved_Model_with_{data_schedule[j]}_CIFAR100_Data_{date_time}")
        model.multi_out = 1
        model.eval()
        for p in model.parameters():
        ...
        with open('{}/Final_Proto_Preds_And_Probs_CIFAR100{}.txt'.format(model_dir, date_time), 'a') as f:
            f.write("\n")
            f.write(
                "Final Preds : {}, \n Final Probs {}  ".format(
                    preds, probs))
        ...
        with open('{}/CS_stats_CIFAR100{}.txt'.format(model_dir, date_time), 'a') as f:
            f.write("\n")
            f.write(
        ...
        for proto in par_image_tensors:
            ...
            with open('{}/Adv_stats_CIFAR100{}.txt'.format(model_dir, date_time), 'a') as f:
                f.write("\n")
                f.write(
    ...
    with open('{}/final_data_summary_CIFAR100{}.txt'.format(model_dir, date_time), 'a') as f:
        f.write(
            "Data \t Test Acc  \t CS_norm metric \t L2 adversarial latent means \t L2 adversarial image means \t CS adversarial image means \t CS adversarial latent means  \n")
        for i in range(len(data_schedule)):
            f.write("{0:4.4f} \t {1:4.4f}\t {2:4.4f}\t {3:4.4f}\t {4:4.4f}\t {5:4.4f}\t {6:4.4f} \n".format(
                data_schedule[i], test_accs[i], CS_means[i], L2_cum_latent_means[i], L2_cum_image_means[i],
                CS_adv_image[i], CS_adv_latent[i]))
    f.close()
    for i in range(len(data_schedule)):
        torch.save(saved_protos[i],
                   f"{saved_protos_path}/{i}_Saved_Protos_with_{data_schedule[i]}_CIFAR100_trainedProtos{date_time}")

    torch.save(model.state_dict(), f"{saved_model_path}/Final_Saved_Model_CIFAR100{date_time}")
    torch.save(par_image_tensors, f"{saved_protos_path}/Final_Saved_Protos_CIFAR100{date_time}")
